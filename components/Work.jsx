      ...
      <div className="border-t rule">
        {featuredProjects.map((p, i) => (
          <Reveal key={p.slug} delay={i * 0.05} y={20}>
            <Link
              href={`/work/${p.slug}`}
              className="group grid grid-cols-[48px_1fr_auto] items-baseline gap-4 border-b rule py-7 transition-colors duration-300 hover:bg-ink hover:text-paper sm:grid-cols-[64px_1fr_auto_90px_auto] sm:gap-8 sm:px-3"
            >
